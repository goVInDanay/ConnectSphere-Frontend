...
export default function StoriesPage() {
  ...
  const handleDeleteStory = async (storyId: number) => {
    try {
      ...
      if (selectedGroup) {
        const updated = selectedGroup.stories.filter(s => s.storyId !== storyId);
    ...
    <AppLayout>
      <div className="space-y-6">
        ...
        <div className="flex items-center justify-between">
          ...
          {user && (
            <>
              <Button size="sm" isLoading={uploading} onClick={() => fileRef.current?.click()}
        ...
        {!user ? (
          <div className="text-center py-20">
            ...
            <h3 className="text-lg font-bold text-foreground mb-2">Sign in to see stories</h3>
            <p className="text-sm text-muted-foreground mb-4">Stories are only visible to logged-in users.</p>
            <Link to="/login"><Button>Sign in</Button></Link>
          </div>
        ) : loading ? (
          <div className="grid grid-cols-2 sm:grid-cols-3 gap-3">
            {[1,2,3,4,5,6].map(i => (
              <div key={i} className="aspect-[9/16] rounded-2xl bg-surface animate-pulse" />
            ))}
          </div>
        ) : groups.length === 0 ? (
          <div className="text-center py-20">
            ...
            <p className="text-sm text-muted-foreground mb-4">
              Be the first to share a story, or follow more people to see theirs.
            </p>
            <Button onClick={() => fileRef.current?.click()} leftIcon={<Upload className="w-4 h-4" />}>
              Share a story
            </Button>
          </div>
        ) : (
          <div className="grid grid-cols-2 sm:grid-cols-3 gap-3">
            {groups.map(group => (
              <StoryCard
                ...
                onClick={() => { setSelectedGroup(group); setSelectedStoryIdx(0); }}
      ...
      {selectedGroup && (
        <div className="fixed inset-0 z-50 bg-black/90 backdrop-blur-sm flex items-center justify-center"
          ...
          <div className="relative w-full max-w-[380px] h-[85vh] rounded-3xl overflow-hidden bg-black shadow-2xl"
            ...
            {/* Story display */}
            <img
              src={selectedGroup.stories[selectedStoryIdx]?.mediaUrl}
              alt="story"
            ...
            <div className="absolute top-4 left-0 right-0 z-10 flex items-center justify-between px-4">
              <div className="flex items-center gap-2">
                <Avatar src={selectedGroup.author?.profilePicUrl} name={selectedGroup.author?.username} size="sm" ring />
                <span className="text-white text-sm font-semibold">@{selectedGroup.author?.username}</span>
              </div>
            ...
            {user?.userId === selectedGroup.userId && (
              <button
                onClick={() => handleDeleteStory(selectedGroup.stories[selectedStoryIdx].storyId)}
            ...
            {selectedGroup.stories.length > 1 && (
              <div className="absolute bottom-16 left-0 right-0 flex justify-center gap-1.5">
                {selectedGroup.stories.map((_, i) => (
                  <button key={i} onClick={() => setSelectedStoryIdx(i)}
...
function StoryCard({ group, isOwn, onClick, onDelete }: {
  group: StoryGroup;
  isOwn: boolean;
  onClick: () => void;
  ...
  return (
    <div className="relative aspect-[9/16] rounded-2xl overflow-hidden cursor-pointer group" onClick={onClick}>
      <img src={latest.mediaUrl} alt="story" className="w-full h-full object-cover transition-transform duration-500 group-hover:scale-105" />
      {/* Gradient overlay */}
      <div className="absolute inset-0 bg-gradient-to-t from-black/80 via-black/10 to-black/20" />
      {/* Story ring indicator */}
      <div className={cn('absolute top-3 left-3 rounded-full p-[2.5px]',
        isOwn ? 'bg-gradient-to-br from-brand-400 to-violet-500' : 'bg-gradient-to-br from-brand-500 to-rose-400')}>
        <div className="bg-black rounded-full p-[2px]">
          <Avatar src={group.author?.profilePicUrl} name={group.author?.username} size="xs" />
        </div>
      </div>
      {/* Story count badge */}
      ...
      <div className="absolute bottom-0 left-0 right-0 p-3">
        <p className="text-white text-xs font-semibold truncate">@{group.author?.username}</p>
        <p className="text-white/60 text-[10px] mt-0.5">{timeAgo(latest.createdAt)}</p>
      </div>
    </div>
  );
}
